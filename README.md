...
Python Neo4j Pinecone React TypeScript TailwindCSS License
...
🚀 Launch Live Demo — Try the app now!

Features • Architecture • Installation • Usage • API • Deployment • License
...
Data Preprocessing (Milestone 1): Clean and structure raw email data
Knowledge Extraction (Milestone 2): Use LLMs to extract entities and relationships
Intelligent Querying (Milestone 3): Hybrid RAG combining graph and vector search
...
Live Demo
🎯 Try the live application now: https://final-mocha-iota.vercel.app
...
Neo4j Instance - Database (AuraDB cloud or local installation)
Pinecone Account - Vector search (free tier available)
Groq/LLAMA API Key - LLM access (free tier available)
Email Dataset - CSV with columns: message_id, body (or body_cleaned), subject
Installation
...
Load raw Enron email dataset
Clean and structure email data
Extract employee information and communications
Generate aggregated communication metrics
Create processed datasets (emails.csv, employees.csv, communications.csv)
Phase 1: Data Preparation
Load processed email dataset into Neo4j as Email nodes
Each email contains: message_id, subject, body
...
Ensure Neo4j instance is running (check AuraDB console)
Test connection: neo4j-admin dbms ping (local) or verify AuraDB status online
...
Create index in Pinecone console or use Pinecone CLI
Set PINECONE_INDEX to match your index name
Run vector indexing: uncomment build_vector_index() in MIlestone-3.py
LLM API Errors
Error: Invalid LLAMA_API_KEY or timeout

Solution:

Verify API key in .env (no spaces, correct value)
Check Groq/LLAMA service status
...
Reduce BATCH_SIZE in Milestone-2.py (e.g., 5 instead of 10)
Reduce MAX_BODY_CHARS (e.g., 2500 instead of 3500)
...
Increase Batch Size (if memory allows): BATCH_SIZE = 20
Use Smaller Models: ministral-3:14b instead of larger ones
Parallel Processing: Run multiple Milestone-2.py instances on different email ranges
Database Indexing: Create Neo4j indexes on frequently queried properties
Caching: Implement response caching for common queries
Deployment
✅ Live Demo (Already Deployed!)
Frontend: https://final-mocha-iota.vercel.app
...
Databases: Use free tiers of Neo4j Aura + Pinecone
...
This software is provided as-is for commercial and personal use. See the LICENSE file for full details.

Support
For issues, questions, or contributions:

Check Troubleshooting section
Review example queries in Example Queries
...
Built with Neo4j, Pinecone, and Groq APIs
Uses the Enron Email Dataset
Powered by LangChain and various LLM providers
Last Updated: March 28, 2026 Version: 1.0.0
